import { emptyResume, type ResumeContent } from "@thomasar-cv/db/schema";
...
describe("validateDraft", () => {
  it("rejects a blank or whitespace-only name", () => {
    expect(validateDraft({ name: "", content: emptyResume }).ok).toBe(false);
    expect(validateDraft({ name: "   ", content: emptyResume }).ok).toBe(false);
  });

  it("rejects content that does not satisfy the schema", () => {
    const bad = { schemaVersion: 2 } as unknown as ResumeContent;
    const result = validateDraft({ name: "CV", content: bad });
  ...
  it("accepts a valid draft and returns the name verbatim", () => {
    ...
    const result = validateDraft({ name: "  My CV ", content: emptyResume });
...
describe("isDirty", () => {
  const base: ResumeDraft = { name: "CV", content: withSummary };

  it("is clean against an equal draft", () => {
    expect(isDirty(base, { name: "CV", content: withSummary })).toBe(false);
  });
  ...
  it("is clean when only key order differs", () => {
    ...
    expect(isDirty(base, { name: "CV", content: reordered })).toBe(false);
  });

  it("is dirty when the name changes", () => {
    expect(isDirty(base, { name: "CV 2", content: withSummary })).toBe(true);
  });
  ...
  it("is dirty when nested content changes", () => {
    ...
    expect(isDirty(base, { name: "CV", content: edited })).toBe(true);
  });
});
