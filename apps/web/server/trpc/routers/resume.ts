...
import { fromJsonResume, jsonResume } from "@thomasar-cv/db/jsonresume";
import { z } from "zod";
...
export const resumeRouter = createTRPCRouter({
   ...
   * export route. The `jsonResume` schema validates the upload, stripping the
   * sections we don't model (so unknown / extra fields drop without failing) and
   * rejecting a malformed shape as BAD_REQUEST *before* anything is written, so a
   * bad import never leaves a partial résumé behind. `fromJsonResume` carries the
   * field correspondence - the same table the export reads. The new row is named
   * from the document's person name (the editor can rename), defaulting when it
   * carries none.
   */
