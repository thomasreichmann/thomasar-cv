 ...
 *            iterating on a single spec. Safe because `setup` is a dependency of
 *            every suite, so Playwright runs it (resetDb) on every invocation -
 *            including a `-g`-narrowed one: grep selects only the primary tests,
 *            but dependency projects always run in full, so the reuse can't
 *            inherit a dirty database.
