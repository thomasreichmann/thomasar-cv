...
- **v0.3 editor + api** - gated on the frontend foundation: settle where components come from, how theming is wired, and the visual baseline before building durable UI on it (shadcn/ui on Base UI, the default shadcn dark theme, dark-only; a bespoke visual identity is a later, deliberate decision; see ADR 0003). With that gate cleared: résumé crud, content editing per section type, reorder + show/hide, live preview reusing the v0.2 engine.
